...
int find_gt_naive(int* vector, int size, int value){
        int index = -1 ;
...
int find_gt_no_break(int* vector, int size, int value){
        int index = -1 ;
        for (int i = 0 ; i < size ; i++){
                if (vector[i] < value){
                        index = i ;
...
int find_gt_compare(int* vector, int size, int value){
        int index = -1 ;
        for (int i = 0 ; i < size ; i++){
...
int find_gt_std_find(int* vector, int size, int value){
        int *pindex = std::find_if(vector, vector + size, [value](int x) {return x > value;}) ;
        // Theorically, we have to verify vector[pindex] == value ...
        return pindex - vector ;
}


int find_gt_intrinsic(int* vector, int size, int value){
        // experimental
        int index = -1 ;
...
// In my cpu : optimal unrolling around 2 SIMD finds per loop
        for (int i = 0 ; i < size ; i+=16){//avx2
                ...
                __m256i chunk2 = _mm256_loadu_si256((const __m256i_u*)&vector[i+8]); // load vector in AVX reg
                                                                                      //
                __m256i cmp  = _mm256_cmpgt_epi32(chunk , target);
                __m256i cmp2 = _mm256_cmpgt_epi32(chunk2, target);
